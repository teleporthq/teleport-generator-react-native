...
import {
  ...
  Generator,
  FileSet
} from "@teleporthq/teleport-lib-js";
...
export default class RNComponentGenerator extends ComponentGenerator {
  ...
  public processStyles(componentContent: any, styles: any): any {
    ...
    if (content.style) {
      const styleName =
        content.name || findNextIndexedKeyInObject(styles, content.type);
  ...
  public computeDependencies(content: any, isPage: boolean): any {
    ...
    if (type) {
        ...
        if (
          ...
          if (childrenDependenciesArray.length) {
            childrenDependenciesArray.forEach(childrenDependency => {
              Object.keys(childrenDependency).forEach(
                childrenDependencyLibrary => {
                  if (!componentDependencies[childrenDependencyLibrary])
      ...
      if (elementMapping) {
        if (elementMapping.source) {
          ...
          if (!dependencies[elementMapping.source])
            dependencies[elementMapping.source] = [];

          // if the type is not yet in the deps for the current library, add it
          if (
            dependencies[elementMapping.source].indexOf(elementMapping.type) < 0
          )
    ...
    if (children && children.length > 0 && typeof children !== "string") {
      const childrenDependenciesArray = children.map(child =>
        this.computeDependencies(child, isPage)
      );
      if (childrenDependenciesArray.length) {
        childrenDependenciesArray.forEach(childrenDependency => {
          Object.keys(childrenDependency).forEach(childrenDependencyLibrary => {
            ...
            dependencies[childrenDependencyLibrary] = union(
              dependencies[childrenDependencyLibrary],
              childrenDependency[childrenDependencyLibrary]
            );
          });
    ...
    if (children && children.length > 0) {
      if (typeof children === "string") {
        if (children.indexOf("$props.") === 0) {
          ...
          if (children.substr(children.length - 1) === " ")
            children += "&nbsp;";
  ...
  public generate(component: any, options: any = {}): FileSet {
    ...
    result.addFile(
      `${component.name}.js`,
      COMPONENTrenderer(name, jsx, dependencies, props, styles)
    );
