...
function renderJsx(
  ...
  const styleString = className ? `style="${className}"` : ''
  ...
  if ((childrenJSX && childrenJSX.length > 0) || isRoot) {
    return `<${name} ${styleString} ${propsString}>
      ${childrenJSX}
    </${name}>`
  } else {
    return `<${name} ${styleString} ${propsString}/>`
...
export default class ReactNativeComponentCodeGenerator extends ComponentCodeGenerator {
